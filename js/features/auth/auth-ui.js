...
export class AuthUI {
  constructor(containerId = 'authUIContainer') {
    ...
    if (!this.container) {
      console.error('[AuthUI] Container not found:', containerId);
      return;
    }
  ...
  openLoginModal() {
    ...
    console.log('[AuthUI] Redirecting to login page');
  }
  ...
  async handleLogout() {
    try {
      ...
      console.log('[AuthUI] Logout successful');
    } catch (error) {
      console.error('[AuthUI] Logout failed:', error);
